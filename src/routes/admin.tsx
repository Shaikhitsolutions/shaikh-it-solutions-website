...
function AdminPage() {
  ...
  const handleWholesaleChange = (val: string) => {
    ...
    if (val && !isNaN(Number(val))) {
      const autoRetail = Math.round(Number(val) * 1.30);
  ...
  const handleDeleteProduct = async (id: number, name: string) => {
    const confirmDelete = window.confirm(`Kya aap sach me "${name}" ko website se delete karna chahte hain?`);
  ...
  return (
    <SiteLayout>
      <div className="relative min-h-screen bg-navy text-navy-foreground overflow-hidden py-24">
        ...
        <div className="relative max-w-4xl mx-auto px-4 sm:px-6 z-10">
          ...
          <div className="text-center mb-12">
            ...
            <h1 className="text-3xl sm:text-4xl font-bold text-white mt-4">Shaikh.IT Master Dashboard</h1>
            <p className="text-navy-foreground/70 mt-2 text-sm">Synchronize active storage units and update structural peripheral info.</p>
          </div>

          {/* Form and Image Wrapper */}
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-12">
            <div className={`lg:col-span-2 glass p-6 rounded-2xl border transition-all duration-300 ${isEditing ? 'border-accent bg-navy-deep/60 shadow-glow' : 'border-white/10 bg-navy-deep/40'}`}>
              ...
              <div className="flex justify-between items-center mb-6 border-b border-white/5 pb-4">
                ...
                {isEditing && (
                  <button onClick={resetFormState} className="flex items-center gap-1 text-xs text-red-400 hover:text-red-300 font-semibold transition-colors">
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div>
                    <label className="block text-xs font-bold uppercase tracking-wider text-navy-foreground/70 mb-1.5">Category</label>
                    <select
                  ...
                  <div>
                    <label className="block text-xs font-bold uppercase tracking-wider text-navy-foreground/70 mb-1.5">Wholesale Cost</label>
                    <input
                      type="text" required value={wholesalePrice}
                  ...
                  <div>
                    <label className="block text-xs font-bold uppercase tracking-wider text-accent mb-1.5">Retail Price (+30%)</label>
                    <input
                      type="text" required value={retailPrice}
                ...
                <button
                  ...
                  className="w-full bg-primary-gradient text-navy-foreground font-bold py-3.5 rounded-xl flex items-center justify-center gap-2 transition-all shadow-card-soft hover:shadow-glow disabled:opacity-40 uppercase text-xs tracking-wider"
                >
                  {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : isEditing ? "Commit Structural Updates" : "Deploy Live Storage Node"}
                </button>
              </form>
            </div>
              ...
              <label className="cursor-pointer w-full h-full flex flex-col justify-center items-center">
                {imagePreview ? (
                  <div className="relative w-full h-44 rounded-xl overflow-hidden bg-navy-deep/80 border border-white/5">
                    ...
                    <span className="absolute bottom-2 left-1/2 -translate-x-1/2 glass-dark text-[9px] font-bold text-white px-3 py-1 rounded-full border border-white/10 uppercase tracking-widest shadow-md">Modify Asset</span>
                  </div>
                ) : (
                  <div className="py-4">
                    <UploadCloud className="h-8 w-8 text-accent/60 mx-auto mb-2" />
                    <span className="text-xs text-white font-medium block uppercase tracking-wider">Upload Asset</span>
                  </div>
                )}
          ...
          <div className="glass p-6 rounded-2xl border border-white/10 bg-navy-deep/40 shadow-xl">
            ...
            {fetchLoading ? (
              <div className="text-center py-10 gap-2 flex items-center justify-center text-navy-foreground/50 text-xs uppercase tracking-wider">
                <Loader2 className="h-4 w-4 animate-spin text-accent" /> Reading Database Nodes...
              </div>
            ) : existingProducts.length === 0 ? (
              <div className="text-center py-8 text-navy-foreground/40 text-xs uppercase">No Active Logs Detected.</div>
            ) : (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="p-2.5 rounded-xl glass-dark text-accent hover:bg-white hover:text-navy transition-all duration-200 border border-white/5"
                      ...
                      <button
                        ...
                        className="p-2.5 rounded-xl bg-red-600/10 text-red-400 hover:bg-red-600 hover:text-white transition-all duration-200 border border-red-500/10"
                        title="Purge Node"
                      >
